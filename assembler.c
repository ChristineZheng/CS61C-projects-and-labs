...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    char buf[BUF_SIZE];
    uint32_t input_line = 1, byte_offset = 0;
    int ret_code = 0;
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        // label
        char* ptr = token;	
        int err = add_if_label(input_line, ptr, byte_offset, symtbl);
	...
	if (err == 1) {
        ...
        int num_args = 0;
        int num_args_error = parse_args(input_line, args, &num_args);
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        char* args[MAX_ARGS];
        int num_args = 0;
        int err2 = parse_args(input_line, args, &num_args);
		if (err2 == -1) {
	    	ret_code = -1;
		}
        int64_t address = byte_offset;

	if ((strcmp(name, "beq") == 0 || strcmp(name, "bne") == 0) && args[2]) {
		address = get_addr_for_symbol(symtbl, args[2]);
	}
	if ((strcmp(name, "j") == 0 || strcmp(name, "jal") == 0) && args[0]) {
		address = get_addr_for_symbol(symtbl, args[0]);
	}

        int error = translate_inst(output, name, args, num_args, address, symtbl, reltbl);
